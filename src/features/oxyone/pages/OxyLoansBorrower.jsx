...
function normalizeBorrowerRow(b) {
  return {
    ...b,
    id: b.userId || b.id || "",
    ...
    email: b.email || "",
    address: b.address || "",
    registeredDate: b.registeredDate || b.createdAt || b.created_at || "",
  };
}
...
export default function OxyLoansBorrower() {
  ...
  const fetchPage = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      const d = res.data ?? {};
      const rows = Array.isArray(d.listOfData) ? d.listOfData.map(normalizeBorrowerRow) : [];
      setData(rows);
      setTotalCount(Number(d.totalCount) || rows.length);
      setPage(Number(d.pageNo) || pg);
    } catch (err) {
      console.error("Borrower API error:", err);
      message.error(err?.response?.data?.message || "Failed to load borrower data. Please try again.");
      setData([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
      ...
      <div className="bg-white border border-slate-200 shadow-sm min-w-0">
        ...
        <div className="px-3 py-1.5 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-bold text-xs text-slate-900">Borrower Records</span>
            {totalCount > 0 && (
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (t) => `Total ${t.toLocaleString()} borrowers`,
