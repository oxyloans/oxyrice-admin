...
export const PRODUCT_COUNTS = [
  ...
  },
  {
    ...
    fetchCount: async () => {
      const res = await adminApi.get("/auth-service/user/registered-users1", {
        params: { pageIndex: 0, pageSize: 1, sortBy: "id", sortOrder: "DESC", status: "live" },
      });
      return toCount(res.data?.count);
    },
    fetchTodayCount: async () => {
      const res = await adminApi.get("/auth-service/user/registered-users1", {
        params: { pageIndex: 0, pageSize: 200, sortBy: "id", sortOrder: "DESC", status: "live" },
      });
  ...
  },
  {
    key: "partner",
    fetchCount: async () => {
      const res = await adminApi.get("/user-service/partners", { params: { page: 0, size: 1 } });
      const d = res.data ?? {};
      return toCount(d.totalElements ?? d.totalCount ?? d.count);
    },
    fetchTodayCount: async () => {
      const res = await adminApi.get("/user-service/partners", { params: { page: 0, size: 200 } });
