...
function normalizePartnerRow(p) {
  return {
    ...p,
    id: p.userId || p.id || "",
    ...
    email: p.email || "",
    address: p.address || "",
    registeredDate: p.registeredDate || p.createdAt || p.created_at || "",
  };
}
...
export default function PartnerLenderUsers() {
  ...
  const fetchPage = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      message.error(err?.response?.data?.message || "Failed to load partner data. Please try again.");
      setData([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
