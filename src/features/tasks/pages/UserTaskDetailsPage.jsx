...
const UserTaskDetailsPage = () => {
  ...
  const selectedMonth =
    location.state?.selectedMonth || dayjs().format("YYYY-MM");
  ...
  const fetchTasksByDate = useCallback(async () => {
    ...
    try {
      ...
      // Filter tasks to include only those from the selected month
      const fetchedTasks = (response.data || []).filter(
        (task) => dayjs(task.planCreatedAt).format("YYYY-MM") === selectedMonth,
      );
