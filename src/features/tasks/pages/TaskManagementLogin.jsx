    ...
    try {
      ...
      if (response.data?.status === "Login Successful") {
        ...
        if (primaryType === "HELPDESKSUPERADMIN") {
          ...
          setTimeout(() => {
            const redirect = localStorage.getItem("redirectAfterLogin_task") || "/taskmanagement/overview";
            localStorage.removeItem("redirectAfterLogin_task");
            navigate(redirect);
          }, 1000);
        } else {
        ...
        <div className="flex border-b">
          <button
            className={`flex-1 py-4 font-medium transition-all duration-300 ${
              activeMode === "live" ? "bg-blue-600 text-white" : "bg-white text-gray-500 hover:bg-gray-50"
            }`}
          ...
          >
            <div className="flex items-center justify-center">
              <div className={`mr-2 rounded-full ${activeMode === "live" ? "bg-blue-200 animate-pulse" : "bg-gray-300"}`}></div>
              Live Environment
            </div>
          </button>
          <button
            className={`flex-1 py-4 font-medium transition-all duration-300 ${
              activeMode === "test" ? "bg-green-600 text-white" : "bg-white text-gray-500 hover:bg-gray-50"
            }`}
          ...
          >
            <div className="flex items-center justify-center">
              <div className={`mr-2 ${activeMode === "test" ? "bg-green-200 animate-pulse" : "bg-gray-300"}`}></div>
              Test Environment
            </div>
          </button>
        </div>
