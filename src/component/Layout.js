...
import {
  FaTachometerAlt, FaClipboardList, FaUsers, FaFolder, FaBox, FaSlideshare,
  FaBoxOpen, FaStore, FaShoppingCart, FaTags, FaFileAlt, FaChartBar, FaExchangeAlt, 
  FaShippingFast, FaReply, FaUser
} from 'react-icons/fa';
...
const sidebarItems = [
  ...
  {
    key: "categories", label: "Categories", icon: <FaFolder />,
...
const MainLayout = ({ children }) => {
  ...
  const ProfileMenu = ({ onSignOut }) => (
    <Menu>
      <Menu.Item key="signout" onClick={onSignOut}>Sign Out</Menu.Item>
    </Menu>
  );
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      {/* Sidebar */}
      <Sider collapsible collapsed={collapsed} onCollapse={setCollapsed} width={230} className="fixed-sidebar">
