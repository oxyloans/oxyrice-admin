...
import { Table, Button, Modal, Form, Input, Row, Col, Select, Switch, message } from "antd";
...
const SubscribersList = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://meta.oxyloans.com/api/erice-service/subscription-plans/getAllSubscriptionForUser", {
        headers: {
  ...
  const columns = [
    ...
    { title: 'Customer Mobile', dataIndex: 'customerMobile', key: 'customerMobile', align: 'center' },
    { title: 'Transaction Id', dataIndex: 'paymentId', key: 'paymentId', align: 'center' },
    {
      title: 'Amount',
      dataIndex: 'amount',
      key: 'amount',
...
const handleSearchChange = (e) => {
  ...
  if (value) {
    ...
    const filtered = subscribers.filter(subscriber =>
      ['customerName', 'customerMobile', 'paymentId', 'amount', 'paymentStatus'].some(key =>
