...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/user-service/write/employeesRegistedCount`);
      if (!response.ok) {
  ...
  const podCompletionData = [
    { name: "POD Posted", value: dashboardData.podPostedEmployeesCount, color: "#10b981" },
    { name: "Remaining POD", value: dashboardData.reamingPODCount, color: "#ef4444" },
  ];

  const eodCompletionData = [
    { name: "EOD Posted", value: dashboardData.eodPostedEmployeesCount, color: "#10b981" },
    { name: "Remaining EOD", value: dashboardData.reamingEODPostCount, color: "#ef4444" },
  ];
  ...
  const podPercentage = (
    (dashboardData.podPostedEmployeesCount / dashboardData.totalRegisteredEmployees) * 100
  ).toFixed(0);

  const eodPercentage = (
    (dashboardData.eodPostedEmployeesCount / dashboardData.totalRegisteredEmployees) * 100
  ).toFixed(0);
    ...
    <TaskAdminPanelLayout>
      <div className="p-2 sm:p-4 md:p-6 max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-2 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Employee Admin Dashboard</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {/* Registered Users */}
          <div
            onClick={() => navigate("/taskmanagement/employee_registered_users")}
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === "Enter" && navigate("/taskmanagement/employee_registered_users")}
            aria-label="View registered users"
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-gray-500">Registered Users</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">{dashboardData.totalRegisteredEmployees}</p>
              </div>
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === "Enter" && navigate("/taskmanagement/employeeleaves")}
            aria-label="View leave requests"
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-gray-500">Leave Requests</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">{leaveCount}</p>
              </div>
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === "Enter" && navigate("/taskmanagement/planoftheday")}
            aria-label="View plan of the day"
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-gray-500">Plan of the Day</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">{dashboardData.podPostedEmployeesCount}</p>
              </div>
            ...
            <div className="mt-3">
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-medium text-gray-700">{podPercentage}%</span>
              </div>
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === "Enter" && navigate("/taskmanagement/endoftheday")}
            aria-label="View end of the day"
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-gray-500">End of the Day</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">{dashboardData.eodPostedEmployeesCount}</p>
              </div>
            ...
            <div className="mt-3">
              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-500">Completion</span>
                <span className="font-medium text-gray-700">{eodPercentage}%</span>
              </div>
              <div className="bg-gray-200 h-2 rounded-full overflow-hidden">
                <div
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{dashboardData.podPostedEmployeesCount} users</p>
                  </div>
                </div>
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{dashboardData.reamingPODCount} users</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{dashboardData.eodPostedEmployeesCount} users</p>
                  </div>
                </div>
                <div className="flex items-center">
                  <div className="w-3 h-3 rounded-full bg-red-500 mr-2"></div>
                  <div>
                    <p className="text-xs text-gray-500">Remaining</p>
                    <p className="text-sm font-medium">{dashboardData.reamingEODPostCount} users</p>
                  </div>
                </div>
              </div>
            </div>
